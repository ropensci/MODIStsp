...
_Note:_ PCs with a small screen can fail to visualise the whole GUI; in this case, the user can add scroll bars with ```MODIStsp(scrollWindow=TRUE)```.

## Selection of processing parameters 

The MODIStsp GUI allows selectiing all processing options required for the creation of the desired MODIS time series. The main available processing options are described in detail in the following.
...
3. **version**: Select wheter processing version 5 or 6 (when available) of MODIS products
...
**Create RsterStack**: Specify if the output time series should be also saved as as "R" _rasterStack_ objects (with temporal information added through the "setZ" method of the _raster_ package). This may be useful in order to easily access the preprocessed MODIS data within "R" scripts.
