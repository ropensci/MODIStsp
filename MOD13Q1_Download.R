...
MOD13Q1_Download <- function(Start_Date=Start_Date,End_Date = End_Date, OutPath = OutPath,  ovr = ovr, del = del, reproj = reproj, opts = opts,
...
# 							while(er_mos != 0 &  er_rep != 0) {
								...
								dir.create(file.path(OutPath, bandnames[band], 'Single_Dates'), showWarnings = F, recursive = T)
								bands = paste(as.character(bands), collapse = '', sep = ' ')					# Convert to character
								outfile = paste(OutPath, '/Mosaic_',substr(Modislist[1],10,16),'_',bandnames[band],'.hdf', sep = '')		# Create name for the HDF mosaic
								...
								if (reproj == T) {
									...
									outrep_file = file.path(OutPath, bandnames[band], 'Single_Dates',paste(sub("[.][^.]*$", "", basename(outfile), perl=TRUE),sep = ''))	# Create name for the TIFF reprojected  mosaic
									...
									if (length(opts$bbox) == 4) {
										...
										er_rep = 	system(paste('gdalwarp -s_srs "',opts$MOD_prj_str[1], 	# Launch GDAL to crete the reprojected TIFF
														...
														'-wo "INIT_DEST=NO_DATA"','-srcnodata ', nodatas[band],'-dstnodata ', nodata_out, '-overwrite',
										...
										er_rep = 	system(paste('gdalwarp -s_srs "',opts$MOD_prj_str[1], 
														...
														'-wo "INIT_DEST=NO_DATA"','-srcnodata ',  nodata_out,'-dstnodata -999',
								...
								if (bandnames [band] == 'QA') {
									QA_File = outrep_file
									er = MOD13Q1_QA_to_QC(OutPath, QA_File)
								}
# 							}
