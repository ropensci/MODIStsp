...
options_file = if (length(Args)>=2) {if (Args[2]=="NULL") NULL else Args[2]} else NULL
...
spatial_file_path = if (length(Args)>=3) {if (Args[3]=="NULL") NULL else Args[3]} else NULL
...
output = MODIStsp(gui=gui, options_file=options_file, spatial_file_path=spatial_file_path)
sink(type = c("output"))
