...
8. Download the newest source file for RGtk2 from https://cran.r-project.org/web/packages/RGtk2/index.html.

9. Assuming that the path to this file is ~/Downloads. Run the following in
terminal (change the path if you did not download in ~/Downloads):

```
~$ cd ~/Downloads
~/Downloads$ R CMD INSTALL RGtk2_2.20.33.tar.gz
```
Note that versions may vary depending on when you download the source file. 
Use the exact filename that sits in the directory (Tip: use tab to help autocomplete).
