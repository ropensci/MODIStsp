...
1. Be sure that R >= 3.2.1 and GDAL >= 1.11.1 are installed.
    ...
    With Ubuntu 15.04 Vivid (or older) and Debian Jessie (or older) it is necessary to install packages from CRAN and UbuntuGIS-unstable repositories (follow instructions [here](https://cran.r-project.org/bin/linux/ubuntu) and [here](https://launchpad.net/~ubuntugis/+archive/ubuntu/ubuntugis-unstable)); with Ubuntu 15.10 Wily (or newer) this step is not mandatory (altough recommended).
...
A description of the different processing parameters to be selected is reported in the "Selection of Processing parameters" subsection.[^2^]
