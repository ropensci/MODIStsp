...
1.  **Select MODIS Tiles**: specify which MODIS tiles need to be processed either by: 
    ...
    a. Using the "Start" and "End" horizontal and vertical sliders in the _Required MODIS Tiles_ frame.  
    b. pressing the __"Select on Map"__ button. A map will open in a browser window, allowing interactive selection of the required tiles
...
2.  **Define Custom Area**: specify a custom spatial extent for the desired outputs either by: 
    ...
    c. pressing the __"Select on Map"__ button. A map will open in a browser window, allowing interactive selection of the spatial extent using the tools on the left
...
<font size="2"> (Note that at launch, **`MODIStsp` _always reloads automatically the processing options used for its last successful run_** </font size="2">.
...
- MODIStsp can be also executed as a "stand-alone" application(i.e., without having to open R/RStudio); to do this, from R launch the function `MODIStsp_install_launcher()`.
...
, where: 
  ...
  - **_Sensor_** indicates to which MODIS sensor the time series belongs ("Terra", "Aqua", "Mixed" or "Combined" (for MCD* products));
  - **_Layer_** is a short name describing the dataset (e.g., b1_Red, NDII, UI);
  - **_ProdCode_** is the code name of the MODIS product from which the image was derived (e.g., MOD13Q1);
