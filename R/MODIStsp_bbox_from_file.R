...
bbox_from_file <- function(file_path, out_crs) {
  ...
  if (class(reference_crs) == "try-error" | (!is.null(attr(reference_gdalinfo, "status")) &
    ...
    if (is.null(attr(reference_ogrinfo, "status"))) {
      ...
      reference_bbox    <- matrix(na.omit(as.numeric(unlist(strsplit(gsub("([^0-9.\\-]+|( - ))+", " ", 
                                                                          reference_ogrinfo[grep("Extent:",
                                                                                                 reference_ogrinfo)] ), " ")))), 
                                  nrow = 2)
    } else if (is.null(attr(reference_gdalinfo, "status"))) {
      reference_gdalinfo <- gdalinfo(file_path)
      reference_bbox     <- cbind(na.omit(as.numeric(unlist(strsplit(gsub("[^0-9.\\-]+", " ",
