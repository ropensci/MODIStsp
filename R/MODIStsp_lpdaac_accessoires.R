...
lpdaac_getmod_dirs <- function(ftp, http, used_server = NA, user = user, 
  ...
  if (!exists("date_dirs")) {
    ...
    # Extract dates
    date_dirs <- strftime(as.Date(gsub(paste0(sel_prod_vers[1], "\\.A(20[0-9][0-9][0-3][0-9][0-9])\\..*"),"\\1",
...
lpdaac_getmod_names <- function(http, ftp, used_server, user, password, date_dir, 
  ...
  if (used_server == "http") {
    ...
    while (class(getlist) == "try-error") {
      ...
      if (class(getlist) != "try-error") {
        getlist <- getlist[grep(".*>([A-Z0-9]+\\.A[0-9]+\\.?[hv0-9]*\\.[0-9]+\\.[0-9]+\\.hdf)<.*",
                                getlist)]
        getlist <- gsub(".*>([A-Z0-9]+\\.A[0-9]+\\.?[hv0-9]*\\.[0-9]+\\.[0-9]+\\.hdf)<.*", "\\1",
                        getlist)
    ...
    getlist <- getlist[grep(paste0(sel_prod_vers[1], "\\.A",
                                   ...
                                   sel_prod_vers[2], "\\.[0-9]+\\.hdf$"), getlist)]
