...
install_MODIStsp_launcher <- function( bin_dir=NA, desktop_dir=NA, desktop_shortcut=TRUE, sudo=FALSE) {
	...
	if (running_os=='Linux') {
		...
		close(fileConn)
		if (sudo) {
			system(paste('sudo -S cp -f', file.path(MODIStsp_dir,'ExtData/Launcher/Bash/MODIStsp.desktop'), file.path(desktop_dir,'MODIStsp.desktop')), input=readline("Enter your password: "))
		} else {
			file.copy(from=file.path(MODIStsp_dir,'ExtData/Launcher/Bash/MODIStsp.desktop'),to=file.path(desktop_dir,'MODIStsp.desktop'),overwrite=TRUE)
		}
	}
	...
	if (running_os=='Windows') {
		...
		if (is.na(bin_dir)) {bin_dir = file.path(Sys.getenv('USERPROFILE'),'AppData/Roaming/Microsoft/Windows/Start Menu/Programs/MODIStsp')}
