#' MODIStsp_set_opts OBSOLETE ! CHANGED TO READING FROM XML FILE ! TO BE REMOVED FROM PROJECCT
#' @description
...
{{prodopts = list()
	...
	prodopts$FTP = hash("Terra" = "http://e4ftl01.cr.usgs.gov/MOLT/MOD09GQ.005/",  # http addresses for data download
											"Aqua" = "http://e4ftl01.cr.usgs.gov/MOLA/MYD09GQ.005/")
...
#- ------------------------------------------------------------------------------- -#
	{{prodopts = list()
			...
			# Original bandnames and characteristics
			prodopts$bandnames =   c("b1_Red", "b2_NIR","b3_Blue","b4_Green","b5_SWIR","b6_SWIR","b7_SWIR" ,"BQ_500", "Sun_Zen","View_Zen","Rel_Az","State_500","DOY")					#Band names (from MRT + additional)
			...
			prodopts$datatype =  	 c("Int16", "Int16", "Int16","Int16","Int16","Int16","Int16","UInt32","Int16","Int16","Int16","UInt16","UInt16")   		#Datatypes for bands
			...
			prodopts$indexes_bandnames =	c('NDVI','EVI','SR','NDFI','NDII6','NDII7','SAVI','RGRI','NDSI','GNDVI')        #Names of possible indexes bands
			prodopts$indexes_nicknames =	c('NDVI',
					'EVI (2.5*((b2_NIR-b1_Red)/(b2_NIR+6*b1_Red-7.5*b3_Blue+1)))',
					'Simple Ratio (b2_NIR/b1_Red)',
					'Flood Index (b1_Red-b7_SWIR)/(b1_Red+b7_SWIR)',
					'NDII6 ((b2_NIR - b6_SWIR) / (b2_NIR + b6_SWIR))',
					'NDII7 (b1_NIR-b7_SWIR)/(b1_NIR+b7_SWIR)',
			...
			prodopts$indexes_formula = 		c('(b2_NIR-b1_Red)/(b2_NIR+b1_Red)',  #Formulas of different indexes - NDVI
					...
					'b2_NIR/b1_Red',  													#SR
					'(b1_Red-b7_SWIR)/(b1_Red+b7_SWIR)',  		  #NDFI
					'(b2_NIR - b6_SWIR) / (b2_NIR + b6_SWIR)',  #NDII6
					'(b2_NIR-b7_SWIR)/(b2_NIR+b7_SWIR)', 			  #NDII7
					'(b2_NIR-b1_Red)/((b2_NIR+b1_Red+0.5)*(1+0.5))', 					#SAVI
					'(b1_Red/b4_Green)',       								  #RGRI
					'(b4_Green-b6_SWIR)/(b4_Green+b6_SWIR)',    #NDSI
					'(b2_NIR-b4_Green)/(b2_NIR+b4_Green)'       #GNDVI
					)
			...
			prodopts$quality_bandnames = c('QA','Int_Snow','MOD35_Snow','Int_Fire','Int_Cld','Cirr','lnd_wat', 'Cl_Sh','MOD35_Cld')        #Names of possible quality bands
			prodopts$quality_nicknames = c('MODLAND QA','Internal Snow Flag', 'MOD35_Snow','Internal Fire Flag', 'Internal Cloud Flag', 'Cirrus Detected','Land/Water Flag', 'Cloud Shadow','MOD35 cloud')        #Names of possible quality bands
			prodopts$quality_source  =	 c('BQ_500','State_500','State_500','State_500','State_500','State_500','State_500','State_500','State_500')        # original band containing the information
			prodopts$quality_bitN = 	  c('0-1','15','12','11','10','8-9','3-5','2','0-1')	# Position of bits of the selected quality indicator in the total "bit word" of the original band
			prodopts$quality_nodata_in =  rep(255, length(prodopts$quality_bandnames))  # nodata in for quality bands (dummy - always 255)
			prodopts$quality_nodata_out =  rep(255, length(prodopts$quality_bandnames)) # nodata out for quality bands (always 255)
			prodopts$quality_bandsel = rep(0, length(prodopts$quality_bandnames))  	 #Selection of desired quality bands (all zeroes)
...
#- ------------------------------------------------------------------------------- -#
#  ---------------Define options for product MOD11A2 -----
#- ------------------------------------------------------------------------------- -#
{{prodopts = list()

		# General options -------
		prodopts$product = "Land_Surf_Temp_8Days_1Km (MOD11A2)"    # Product Name
		prodopts$main_out_folder = "Land_Surf_Temp_8Days_1Km"          # output folder Prefixes
		prodopts$native_res = 926.6254
		prodopts$file_prefix = hash("Terra" = "MOD11A2","Aqua" = "MYD11A2")     # Filename Prefixes
		prodopts$FTP = hash("Terra" = "http://e4ftl01.cr.usgs.gov/MOLT/MOD11A2.005/",  # http addresses
				"Aqua" = "http://e4ftl01.cr.usgs.gov/MOLA/MYD11A2.005/")

		# Original bandnames and characteristics ---
		prodopts$bandnames =  c('LST','LST_QC',  'LST_Time', 'View_Zen','LST_Night','LST_Night_QC','LST_Time_Night','View_Zen_Night','Emiss_B31','Emiss_B32','Days_Clear','Nights_Clear')					#Band names (from MRT )
		prodopts$nicknames =  c('Surface Temperature','LST Quality',  'LST Time', 'Sensor Zenith','Surface Temperature - Night','LST Night Quality','LST Night Time','Sensor Zenith Night','Emissivity b31','Emissivity b32','Cloud Coverage Day','Cloud Coverage Night')  				#Band names (from MRT ) #Band nicknames (used for folder creation and band selection)
		prodopts$datatype =   c("UInt16", "Byte","Byte","Byte","UInt16","Byte","Byte","Byte","Byte","Byte","UInt16","UInt16")   #Datatypes for bands
		prodopts$nodata_in =  c(0,-999,0,255,0,-999,0,255,0,0,0,0)  # Nodata values in input MODIS Science DataSets	# Nodata values in input MODIS Science DataSets
		prodopts$nodata_out = c(0,255,255,255,0,255,255,255,0,0,0,0)          # Nodata values in output images
...
{{prodopts = list()
  ...
  prodopts$datatype =  c("Int16", "Int16","UInt16","Int16","Int16","Int16","Int16","Int16","Int16","Int16","Int16","Byte")   #Datatypes for bands
	...
	# Derived Indexes and Characteristics
	prodopts$indexes_bandnames =	c('SR','NDFI','NDII7','SAVI','RGRI')        #Names of possible indexes bands
