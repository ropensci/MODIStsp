...
#' \dontrun{
#'	in_file = 'path_to_file/MOD13Q1_MYD13Q1_NDVI_49_2000_353_2015_RData.RData'  # Input rts file
#'	in_rts = get(load(in_file))
#'	start_date = as.Date("2010-01-01")  # Start date for extraction
#'	end_date = as.Date("2014-12-31")    # End date for extraction
#'	shp_name = 'path_to_file/MODIS_Paper/rois.shp'  # Polygon Shapefile for which time series has to be extracted
#'	shape = readOGR(dirname(shp_name), 'rois')
#'	data_avg = MODIStsp_extract(in_rts, start_date, end_date, shape, FUN = 'mean', na.rm = T, verbose = T)
#'	data_sd = MODIStsp_extract(in_rts, start_date, end_date, shape, FUN = 'sd',    na.rm = T, verbose = T)
#'	plot.xts(data_avg)
#' }
#'
...
#				ts[f,] = extract(in_rts@raster[[sel_indexes[f]]], sp_object, mean, na.rm = T)
#			browser()
			}
		}
