...
update_bboxlabels <- function(bbox_out,
  ...
  svalue(output_ul_east_wid)  <- formatC(bbox_out[1, 1],
                                         digits = digits,
                                         format = "f")
  svalue(output_ul_north_wid) <- formatC(bbox_out[2, 2],
                                         digits = digits,
                                         format = "f")
  svalue(output_lr_east_wid)  <- formatC(bbox_out[1, 2],
                                         digits = digits,
                                         format = "f")
  svalue(output_lr_north_wid) <- formatC(bbox_out[2, 1],
                                         digits = digits,
                                         format = "f")
  #nocov end
}
...
#' @title update_tiles
#' @description Helper function to update the selected tiles with the intersection with ####
#'  the bounding box
