...
moddwl_QA_convert <- function(out_prod_folder = out_prod_folder, QA_file = QA_file, product = product, out_format = out_format) {
  ...
  in_raster = raster(QA_file, format = out_format)				# Open QA file
  ...
  writeRaster(out_qc_raster,QC_file, format = out_format ,overwrite = TRUE, datatype='INT1U')
