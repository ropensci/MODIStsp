...
MODIStsp_read_xml <- function(prodopts_file = prodopts_file, xml_file = xml_file) {
  ...
  names_products <- names(xmldata) # names of the single products
  ...
  for (prod in names_products) {
    ...
    for (n_version in 1:n_versions) {
      ...
      levels(datatype)[levels(datatype)== "16-bit unsigned integer"] <- "UInt16"
      levels(datatype)[levels(datatype)== "32-bit signed integer"]   <- "Int32"
      levels(datatype)[levels(datatype)== "32-bit unsigned integer"] <- "UInt32"
