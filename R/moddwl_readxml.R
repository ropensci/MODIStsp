...
moddwl_read_xml_opts = function(previous_file = previous_file, xml_file = xml_file) {
	...
	for (prod in 1:n_products)  # cycle on available products
	{
		...
		if (nindexes > 0 ) {
			indexes_bandnames = NULL  ;	indexes_fullnames = NULL
			indexes_formulas = NULL   ;	indexes_nodata_out = NULL
		...
		if (nquality > 0 ) {
			quality_bandnames = NULL    ;	quality_fullnames = NULL
			quality_source = NULL   ;    	quality_bitN = NULL
