...
moddwl_main = function(gui=TRUE, options_file=NULL, moddwl_dir=NA) {
...
#					'raster','RCurl','stringr','tools','rts','RGtk2','gWidgetsRGtk2','spatial.tools', 'gdalUtils',')
	...
	setwd(main_dir);   main_dir = getwd()   ; log_dir =  file.path(main_dir,'Log')   ; log_file =  file.path(log_dir,'Log.txt')
	...
	source(file.path(src_dir,'moddwl_check_files.R'))
...
#- ------------------------------------------------------------------------------- -#
	{out_proj_names = c("Sinusoidal","UTM 32N","Latlon WGS84","User Defined" )
		...
		general_opts = list(main_dir = main_dir, previous_file=previous_file,xml_file = xml_file,  log_file = log_file, out_proj_list = out_proj_list, out_proj_names = out_proj_names, MOD_proj_str = MOD_proj_str, 
