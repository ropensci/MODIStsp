 ...
 # xml_file = file.choose()
  ...
  MODIStsp_addindex = function(option_file, gui=TRUE, new_indexbandname="", new_indexfullname="", new_indexformula="", new_indexnodata_out = '32767') {
	...
	check_formula_errors <- function(new_indexbandname, new_indexfullname, new_indexformula, n_products, prod_opt_list, refbands_names) {
		...
		} else {catch_err = 1} # error 1: error in the formula
	...
	if (gui) {
	  	...
	  	sel_indexbandname <- gedit(text = new_indexbandname, label = "Please Insert a valid Proj4 string				", container=indexbandname_group, size = 800, horizontal = T)
