...
-   [MODIStsp: a "R" package for preprocessing MODIS time series](#modistsp-a-r-package-for-preprocessing-modis-time-series)
...
MODIStsp: a "R" package for preprocessing MODIS time series
===========================================================
...
-   MODIStsp **requires R v &gt;= 3.2.1 and GDAL (Geospatial Data Abstraction Library) with HDF4 support** to be installed in your system. Brief instructions for installing R and GDAL can be found HERE

-   Solutions to common **installation, data download and processing problems** can be found in MODIStsp [FAQ](https://github.com/lbusett/MODIStsp/wiki/)
...
Scheduled processing can be used to periodically and automatically update the time series of a selected product over a given study area. To do that, you should simply:
...
### Standalone execution (experimental)

MODIStsp can be also executed as a standalone application; to do that, from R launch the function `MODIStsp_install_launcher()`. In a Linux operating system this function creates a desktop entry (accessible from the menu in the sections "Science" and "Geography") and a symbolic link in a known path (default: /usr/bin/MODIStsp). In Windows, a link in the Start Menu and optionally a desktop shortcut are created. See `?install_MODIStsp_launcher` for details and path customization.
