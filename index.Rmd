...
- 09/05/2020 - MODIStsp 1.4.0 is out. Switches to use of GDAL3/PROJ6 WKTs for projection representation and usage of `sf` for all internal work on vector data. Adds upport for products MCD19A1 and MCD19A2 products.
...
- 29/11/2018 - We recently discovered a nasty bug in the computation of some custom spectral indices (those including additions / subtractions on reflectance values, such as in $\frac{(b1_{NIR}+0.1)}{b2_{Red}}$ (with $\rho$ being a reflectance). See [here](articles/discovered_bug.html) for further details! The bug is now fixed on the 
